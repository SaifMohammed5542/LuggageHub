...
export async function fetchWithAuth(url, token, opts = {}) {
  ...
  if (res.status === 401 || res.status === 403) {
    ...
    const isExpired =
      message && (String(message).toLowerCase().includes("expire") || String(message).toLowerCase().includes("token expired") || body?.expired === true);
    ...
    if (isExpired) {
      try {
        ...
        if (typeof window !== "undefined") {
          ...
          // optional: any other stored keys you use
          // redirect to login
